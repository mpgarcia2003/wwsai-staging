        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          <div className="flex items-center gap-6">
            <a href={SHOPIFY_URL} target="_top" className="text-xl font-bold tracking-tight flex items-center gap-1 group">
              World Wide <span className="text-indigo-600">Shades</span>.
              <ExternalLink size={12} className="text-slate-300 group-hover:text-indigo-400 transition-colors" />
            </a>
            
            {currentPage === 'admin' && (
              <button onClick={() => onNavigate('builder')} className="text-sm font-bold text-indigo-600 hover:underline">
                Back to Visualizer
              </button>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button 
                ...
                className="hidden md:flex items-center gap-1.5 text-slate-600 hover:text-indigo-600 font-bold text-xs mr-2 border border-gray-200 px-2 py-1 rounded transition-colors"
            ...
            <button onClick={onOpenCart} className="relative p-2 rounded-full transition-colors hover:bg-gray-100">
               ...
               {cartCount > 0 && (
                 <span className="absolute top-0 right-0 bg-indigo-600 text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full animate-in zoom-in font-bold">
                   {cartCount}
                 </span>
               )}
            </button>
